            ...
            public boolean onQueryTextChange(String s) {
                ...
                if (!TextUtils.isEmpty(s)) {
                    ...
                    for (int i = 0; i < animents.size(); i++) {
                        if (animents.get(i).getName().contains(s))
                            filted.add(animents.get(i).getName());
                    }
...
//        选中动漫后，底部的两个recycleview.
        ...
        if (Build.VERSION.SDK_INT > 22)
            floatingActionButton.setBackgroundTintList(getColorStateList(R.color.colorRED));
    }
...
//            String[] animentString = title.split("\n");//每一行对应一条目录
        Gson gson = new Gson();
//
        animents = gson.fromJson(intent.getStringExtra("directory"), new TypeToken<ArrayList<animent>>() {
        }.getType());
//            animents =
...
//            List<ImageBannerEntry> items = new ArrayList<>();
        ...
        List<ImageBannerEntry> items = gson.fromJson(intent.getStringExtra("banner"), new TypeToken<List<ImageBannerEntry>>() {
        }.getType());
